...
function TitulosConciliadosPesquisados({ props }) {
  ...
  const urlApiTitulosConciliados = `${BASE_URL}/vendas/conciliadas`;
  
  var config = {
    method: "POST",
    url: `${urlApiTitulosConciliados}?size=12&page=${paginaNumero}&sort=dataEnvio`,
    headers: {
  ...
  return (
    <div className={styles.titulos_container}>
      <div className={styles.titulos_container_cabecalho}>
        <div className={styles.titulos_titulo}>Títulos Conciliados</div>
      </div>
...
export default TitulosConciliadosPesquisados;
